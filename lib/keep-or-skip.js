...
var debugHandler = helpers.debugHandler
var inputParser = helpers.inputParser
var wrapperMakers = helpers.wrapperMakers
var makePredicateMiddleware = wrapperMakers.makePredicateMiddleware
var makeWrappedMiddleware = wrapperMakers.makeWrappedMiddleware
...
function keepOrSkip(middlewares, predicate, debug) {
    ...
    debugHandler.setLocalDebug(debug)
    var predicateWrapper = makePredicateMiddleware(predicate)
    var middlewareWrappers = parsedMiddlewares.map(makeWrappedMiddleware)
    return [predicateWrapper].concat(middlewareWrappers)
}

keepOrSkip.debug = debugHandler.setGlobalDebug
