 ...
 * @param {Function} predicate A middleware that returns a boolean value by
 * which one or more middlewares will be executed or skipped.
...
module.exports = function keepOrSkip(middlewares, predicate) {
    if (!middlewares) {
        throw new KeepOrSkipError('The middlewares parameter must be set.')
    }
    ...
    if (isFunction(middlewares)) {
        ...
        if (middlewares.length === 0) {
            throw new KeepOrSkipError('The middlewares parameter must be' +
                ' a function or an array of functions.' +
                ' More info at: http://expressjs.com')
        }
        middlewares.forEach(function (m) {
            if (!isFunction(m)) {
                throw new KeepOrSkipError('The item ' + m + ' in middlewares' +
